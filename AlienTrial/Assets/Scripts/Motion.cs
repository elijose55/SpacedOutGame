...
public class Motion : MonoBehaviour
{
    ...
    IEnumerator stepWaiter(float delay)
    {
    ...
    private void Update()
    {
        if(!dead)
        {
            ...
            if((t_hmove != 0 || t_vmove != 0) && isGrounded) {
                if(!audioSourceMain.isPlaying && !step) {
                    ...
                    if (isSprinting) StartCoroutine(stepWaiter(0.4f));
                    else StartCoroutine(stepWaiter(0.5f));
    ...
    void FixedUpdate()
    {
        if(!dead)
        {
            ...
            if (isSprinting)
            {
                ...
                cam.fieldOfView = Mathf.Lerp(cam.fieldOfView, baseFOV, Time.deltaTime * 6f);
            }
